...
const AddEstate = () => {
  ...
  useEffect(() => {
    if (dataEstate?.getEstate) {
      ...
      setValue("garden", dataEstate?.getEstate?.garden || "");
      setValue("pool", dataEstate?.getEstate?.pool || "");
      setValue("credit", dataEstate?.getEstate?.credit || "");
      setValue("commercial_use", dataEstate?.getEstate?.commercial_use || "");
      setValue("has_cartel", dataEstate?.getEstate?.has_cartel || "");
      setValue("pets", dataEstate?.getEstate?.pets || "");
    ...
    <div className="w-full flex justify-center mt-5">
      ...
      >
        ...
        <div className="mt-3 mx-auto grid grid-cols-2 gap-x-4 mb-10">
          ...
          />
          <Button label={"Cancelar"} type="button" bgColor={"bg-red-500"} />
        </div>
      </form>
    </div>
  );
